...
const Publications: React.FC = () => {
  ...
  const handleYouTubeAuth = async () => {
    try {
      ...
      const response = await fetch('https://fitoffice2-ff8035a9df10.herokuapp.com/api/entrenadores/google/auth', {
        method: 'GET',
        headers: {
  ...
  useEffect(() => {
    const fetchSocialAuthStatus = async () => {
      try {
        ...
        const response = await fetch('https://fitoffice2-ff8035a9df10.herokuapp.com/api/entrenadores/auth/social/status', {
          method: 'GET',
          headers: {
            Authorization: `Bearer ${token}`,
            'Content-Type': 'application/json'
